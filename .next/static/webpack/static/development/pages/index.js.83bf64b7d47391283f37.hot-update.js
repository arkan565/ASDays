...
function (_React$Component) {
  ...
  _createClass(Index, [{
    ...
    value: function getNext(activities) {
      var date = new Date();
      var active = activities.filter(function (activity) {
        return activity.active == true;
      });

      if (active.length == 3) {
        this.nextActivities = active;
      } else if (active.length == 2) {
        activities.forEach(function (activity) {
          var splitStartDate = activity.startTime.split(':');
          var startHour = parseInt(splitStartDate[0], 10);

          if (!activity.active && startHour > date.getHours() && active.length < 3) {
            active.push(activity);
          }
        });
      } else if (active.length == 1) {
        if (activities[0].active) {
          active.push(activities[1]);
          active.push(activities[2]);
        } else if (activities[activities.length - 1].active) {
          active.push(activities[activities.length - 2]);
          active.push(activities[activities.length - 3]);
        } else {
          for (var i = 0; i < activities.length; i++) {
            if (activities[i].active) {
              active.push(activities[i + 1]);
              active.push(activities[i + 2]);
              break;
            }
          }
        }
...
//# sourceMappingURL=index.js.83bf64b7d47391283f37.hot-update.js.map